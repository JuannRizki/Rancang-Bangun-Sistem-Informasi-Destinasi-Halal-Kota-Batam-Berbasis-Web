...
export default function EditDestinasiPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/destinasi/${id}`, {
      headers: {
      ...
      .then((data) => {
        setForm({
          ...
          alamat: data.alamat || "",
          latitude: data.latitude?.toString() || "1.1291",
          longitude: data.longitude?.toString() || "104.7313",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(
          "Pembaruan berhasil disimpan. Data Anda sedang menunggu persetujuan Admin kembali."
        );
        router.push("/umkm/destinasi");
      } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <h1 className="mb-8 text-2xl font-semibold">Edit Usaha</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Foto Usaha (Kosongkan jika tidak ingin mengubah)
            </label>
            <input
              type="file"
              accept="image/*"
          ...
          <div className="flex gap-4 pt-4">
            <button
              type="button"
              onClick={() => router.push("/umkm/destinasi")}
